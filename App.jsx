      ...
      <section id="portfolio" className="py-20 px-6 bg-[#051f20]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-4 mb-8">
            {portfolioSamples.filter(sample => sample.channel === selectedChannel).map((sample, idx) => (
              <button
                key={idx}
                onClick={() => {
          ...
          <div className="bg-[#1a4f52] rounded-lg shadow-xl p-8 md:p-12 border border-[#5a8a70]/30">
            <div className="mb-6">
              <div className="inline-block bg-[#5a8a70]/30 text-[#97c4a8] px-3 py-1 rounded-full text-xs font-medium mb-3 border border-[#5a8a70]/40">
                {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.type}
              </div>
              <h3 className="text-2xl font-bold baskerville mb-2 text-[#97c4a8]">
                {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.channel} | {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.brand}
              </h3>
            </div>
            ...
            <div className="border-l-4 border-[#5a8a70] pl-6 mb-6">
              ...
              <div className="text-xl font-semibold mb-4 text-white">
                {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.subject}
              </div>
              <div className="text-sm text-white/60 mb-2">Preview Text</div>
              <div className="text-white/80 leading-relaxed mb-6">
                {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.preview}
              </div>
            ...
            {showFullEmail && (
              <div className="bg-[#0d3d3f] p-6 rounded-lg border border-[#5a8a70]/30 mb-6">
                <div className="text-xs text-[#97c4a8] uppercase tracking-wider mb-3">Full {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.channel}</div>
                <div className="text-white/80 leading-relaxed whitespace-pre-line font-mono text-sm">
                  {portfolioSamples.filter(s => s.channel === selectedChannel)[selectedSample]?.fullEmail}
                </div>
              </div>
            )}
          </div>
